    ...
    <div class="adding-post container">
        <div class="adding-post__tabs-wrapper tabs">
            ...
            <div class="adding-post__tab-content">
                <section class="adding-post__<?= $adding_type; ?> tabs__content tabs__content--active">
                    ...
                    <form class="adding-post__form form" action="add.php?type=<?= $adding_type; ?>" method="post"<?=($adding_type === 'photo') ? ' enctype="multipart/form-data"' : ''; ?>>
                                ...
                                <?php if ($field['field_type'] !== 'input-file'): ?>
                                    <div class="adding-post__<?= $field['field_type']; ?>-wrapper form__<?= $field['field_type']; ?>-wrapper">
                                        ...
                                        <?php if ($field['field_type'] === 'textarea'): ?>
                                            <textarea class="adding-post__textarea<?= ($adding_type === 'quote') ? ' adding-post__textarea--quote' : ''; ?> form__textarea form__input" id="<?= $field['name']; ?>" name="<?= $field['name']; ?>" placeholder="<?= $field['placeholder'] ?? ''; ?>"><?= isset($_POST[$field['name']]) ? ($_POST[$field['name']]) : ''; ?></textarea>
                                        <?php elseif ($field['field_type'] === 'input'): ?>
                                            <input class="adding-post__input form__input" id="<?= $field['name']; ?>" type="<?= $field['type']; ?>" name="<?= $field['name']; ?>" placeholder="<?= $field['placeholder'] ?? ''; ?>" value="<?= isset($_POST[$field['name']]) ? $_POST[$field['name']] : ''; ?>">
                                        <?php endif; ?>
                        ...
                        <?php if ($field['field_type'] === 'input-file'): ?>
                            <div class="adding-post__input-file-container form__input-container form__input-container--file">
                                <div class="adding-post__input-file-wrapper form__input-file-wrapper js-file-error form__input-section<?= isset($errors[$field['name']]) ? ' form__input-section--error' : ''; ?>"<?= isset($errors[$field['name']]) ? ' style="border: 2px solid #f02323!important;border-radius: 10px!important;"' : ''; ?>>
